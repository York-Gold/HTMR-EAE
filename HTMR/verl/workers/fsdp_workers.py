...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def init_model(self):
        ...
        if self._is_actor:
            hybrid_token_masking = self.config.actor.get("hybrid_token_masking", None)
            hybrid_tokenizer_path = self.config.actor.get("hybrid_tokenizer_path", None)
            if hybrid_token_masking is not None:
                OmegaConf.set_struct(self.config.actor, False)
                with open_dict(self.config.actor):
                    self.config.actor.hybrid_token_masking = hybrid_token_masking
            if hybrid_tokenizer_path is not None:
                OmegaConf.set_struct(self.config.actor, False)
                with open_dict(self.config.actor):
                    self.config.actor.hybrid_tokenizer_path = hybrid_tokenizer_path
