...
from verl.trainer.ppo.core_algos import (
    ...
    get_global_entropy_top_and_key_mask,
    get_policy_loss_fn,
    kl_penalty,
)
            ...
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    if entropy_top_ratio is not None:
                        hybrid_token_masking = self.config.get("hybrid_token_masking", False)
                        hybrid_tokenizer_path = self.config.get("hybrid_tokenizer_path", None)
                        if not hybrid_token_masking:
                            entropy_top_mask = get_global_entropy_top_mask(entropy=entropy, response_mask=response_mask, top_ratio=entropy_top_ratio)
                        elif hybrid_token_masking:
                            entropy_top_mask = get_global_entropy_top_and_key_mask(entropy=entropy, response_mask=response_mask,  data=micro_batch, top_ratio=entropy_top_ratio, tokenizer_path=hybrid_tokenizer_path)
