...
def get_global_entropy_top_and_key_mask(entropy, response_mask, data=None, top_ratio=0.2, tokenizer_path=None):
    if tokenizer_path is None:
        raise ValueError("tokenizer_path must be provided for hybrid token masking.")
    from transformers import AutoTokenizer
    tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, use_fast=True)
    # event-critical tokens
    key_response_mask = get_key_token_mask(data, tokenizer)
    # high-entropy tokens
    entropy_top_mask = get_global_entropy_top_mask(entropy, response_mask, top_ratio=top_ratio)
    # combine two masks
    combined_mask = torch.maximum(entropy_top_mask, key_response_mask)
    return combined_mask

# extract event-critical tokens from response
def get_key_token_mask(data, tokenizer):
    input_ids = data.batch["input_ids"]
    attention_mask = data.batch["attention_mask"]
    response_ids = data.batch["responses"]
    response_mask = data.batch["response_mask"]

    # first decode response_ids to text
    response = tokenizer.batch_decode(response_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)
    key_response_mask = torch.zeros_like(response_ids)
    # second extract event-critical tokens
    for batch_idx, (res_text, res_ids, res_mask) in enumerate(zip(response, response_ids, response_mask)):
        try:
            model_res = extract_ground_truth(res_text)
            if model_res is None:
                continue
            # extract entities as event-critical tokens (strictly supports strings only)
            entity_list: list[str] = []
            # extract roles as event-critical tokens (strictly supports strings only)
            role_list: list[str] = []
            for ans in model_res:
                if "entity" in ans:
                    ent = ans["entity"]
            ...
            # extract all entities' event-critical tokens
            for entity in entity_list:
                entity_tokens = tokenizer.encode(entity, add_special_tokens=False)
                positions = find_all_sublists(res_ids.tolist(), entity_tokens)
                for (start, end) in positions:
                    key_response_mask[batch_idx, start:end] = 1
            # extract all roles' event-critical tokens
            for role in role_list:
                role_tokens = tokenizer.encode(role, add_special_tokens=False)
                positions = find_all_sublists(res_ids.tolist(), role_tokens)
                for (start, end) in positions:
                    key_response_mask[batch_idx, start:end] = 1
            star_positions = []
            in_star = False
            start_idx = 0
            for idx, token_id in enumerate(res_ids.tolist()):
                ...
                if '*' in token and '**' not in token:
                    if not in_star:
                        ...
                        in_star = False
                        star_positions.append((start_idx, idx + 1))  
            for (start, end) in star_positions:
                key_response_mask[batch_idx, start:end] = 1
            # intersect with response mask
            key_response_mask[batch_idx] = key_response_mask[batch_idx] * res_mask
        except Exception as e:
            continue
    return key_response_mask

def extract_ground_truth(prompt: str):
    start_token = "```json"
    end_token = "```"
    start_idx = prompt.rfind(start_token)
    end_idx = prompt.rfind(end_token)
    if start_idx != -1 and end_idx != -1 and end_idx > start_idx:
        json_str = prompt[start_idx + len(start_token) : end_idx].strip()
        try:
...
def find_all_sublists(response_ids, pattern):
    """Find all occurrences of a sublist (pattern) in a list (response_ids). 
    
    Args:
        response_ids: List of token ids representing the response.
        pattern: List of token ids representing the sublist to find.
        
    Returns:
        positions: List of tuples indicating the start and end indices of each occurrence.
    """
    n, m = len(response_ids), len(pattern)
    positions = []
    if m == 0 or n < m:
        return positions
    
    for i in range(n - m + 1):
        if response_ids[i:i+m] == pattern:
